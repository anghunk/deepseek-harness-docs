...
`0.1.3-alpha.2` 起，当 agent 的 provider/model 在会话中途发生变化时，循环会在下一个 admitted request 中自动追加一条 user-role 通知消息。这条消息的格式为：

```
[model changed: assistant turns above this point were generated by ${from}; the session continues with ${to}]
```

其中 `from` 和 `to` 是 provider/model 的路径标签（如果 provider 相同则只显示 model，否则显示 `provider/model`）。

**实现机制**：`installModelSelection()` 在 `agent/pre-step` waterfall 中注册监听器（`packages/core/agent/src/model-selection.ts`），比较 `selection.assembled`（当前步骤组装的模型选择）与 `agent.session.requestHeader()?.config`（上一个请求的模型配置）。如果 provider 或 model 发生变化且不是 effort-only 变化，则在 `PreStepDecision` 的 messages 数组末尾追加这条通知。通知的 `source` 标记为 `{ kind: 'plugin', plugin: 'model-selection', form: 'notice' }`，summary 为 `from → to`。

**设计要点**：
- 通知仅在 provider/model 变化时插入，effort-only 变化不触发
- 失败的请求在 header 持久化前会重复触发通知（下次请求再次插入）
- 空的 no-request 决策（step 1 或 messages 非空时）跳过通知
- 通知是 durable 的 user-role 消息，写入会话日志，模型可见
